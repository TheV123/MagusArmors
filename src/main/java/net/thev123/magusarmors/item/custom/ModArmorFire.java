...
public class ModArmorFire extends ArmorItem {
    private final Multimap<EntityAttribute, EntityAttributeModifier> attributeModifiers;
    public ModArmorFire(ArmorMaterial material, Type type, Settings settings) {
        super(material, type, settings);

        float fireSpellPower = 0.2F;
        float hastePower = 0.05F;
        float critDamagePower = 0.03F;
        float critChancePower = 0.02F;

        int protection = ModArmorMaterials.FIRE_MATERIAL.getProtection(type);
        float toughness = ModArmorMaterials.FIRE_MATERIAL.getToughness();
